...
public struct PoolingConfiguration: Codable {
    ...
    /// Optional because some models (e.g. sentence-transformers/all-MiniLM-L6-v2)
    /// omit this field from their pooling config. Defaults to false.
    public let poolingModeLastToken: Bool

    /// Whether to use the mean of the square root of the token length for pooling.
    /// Optional because most models omit this field. Defaults to false.
    public let poolingModeMeanSqrtLenTokens: Bool
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case poolingModeMeanSqrtLenTokens = "pooling_mode_mean_sqrt_len_tokens"
    }

    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.dimension = try container.decode(Int.self, forKey: .dimension)
        self.poolingModeClsToken = try container.decode(Bool.self, forKey: .poolingModeClsToken)
        self.poolingModeMeanTokens = try container.decode(Bool.self, forKey: .poolingModeMeanTokens)
        self.poolingModeMaxTokens = try container.decode(Bool.self, forKey: .poolingModeMaxTokens)
        self.poolingModeLastToken = try container.decodeIfPresent(Bool.self, forKey: .poolingModeLastToken) ?? false
        self.poolingModeMeanSqrtLenTokens = try container.decodeIfPresent(Bool.self, forKey: .poolingModeMeanSqrtLenTokens) ?? false
    }
}
